...
def get_questions():
    ...
    if len(formatted_qs) == 0:
        ...
        return jsonify({
            'success': True, 'questions': formatted_qs,
...
def get_category_questions(category_id):
    ...
    if len(questions) == 0:
        ...
        return jsonify({
            'success': True, 'questions': formatted_qs,
            'total_questions': len(questions), 'current_category': category_id
        })
...
def add_or_search_questions():
    ...
    if 'searchTerm' in request_data:
        ...
        return jsonify({
            'success': True, 'questions': formatted_qs,
            'total_questions': len(questions), 'current_category': None
        })
    else:
        try:
            # form category is string JS object key but needs to be int for DB
            request_data['category'] = int(request_data['category'])
            new_question = Question(**request_data)
            new_question.insert()
            return jsonify(
                {'success': True, 'new_question_id': new_question.id})
...
def quiz():
    ...
    Should return a new question (not previously seen) from chosen category.
    """
    request_data = request.get_json()
    # no category chosen
    if not request_data.get("quiz_category") or int(
            request_data.get("quiz_category").get("id")) == 0:
        questions = Question.query.all()
    else:
        category = Category.query.get(int(request_data['quiz_category']['id']))
        questions = category.questions
    remaining_qs = [
        q for q in questions if q.id not in request_data["previous_questions"]
    ]
    ...
    if remaining_qs:
        ...
        return jsonify({
            'success': True, 'question': next_q, 'previous_questions': prev_qs
        })
...
def unprocessable(error):
    return jsonify(
        {'success': False, 'error': 422, 'message': 'Unproceessable entity'})
